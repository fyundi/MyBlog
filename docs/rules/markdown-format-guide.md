...
3. **保持文本连贯性，避免频繁使用内联代码打断文本**
   ...
   说明：当同一个概念在段落中多次出现时，第一次可以使用内联代码，后续出现应使用普通文本，保持文本流畅性。只有在需要特别强调或区分技术术语时才使用内联代码。
...
2. **在列表项中强调方法名、模式名等概念时，使用加粗而非内联代码**
   ```markdown
   ...
   1. **避免 async void**：除了事件处理，尽量使用 **async UniTask**
   ```
   说明：内联代码用于具体的代码片段、命令、文件名等；加粗用于强调概念、模式、方法名等，避免错误的代码块渲染。
...
3. **在列表项或注意事项中，使用加粗而非内联代码强调概念**
   ```markdown
   1. **避免 async void**：除了事件处理，尽量使用 **async UniTask**
   2. **使用 CancellationToken**：长时间操作必须支持取消
   ```
   说明：在列表项、注意事项、最佳实践等场景中，如果要强调的是一个概念、模式或方法名，而不是具体的代码片段，应使用加粗而非内联代码。这样可以避免错误的代码块渲染，同时保持文本流畅性。
...
2. **在列表项中避免使用内联代码强调概念**
   ...
   1. **避免 async void**：除了事件处理，尽量使用 **async UniTask**
   ```
   说明：在列表项中，如果要强调的是方法名、模式名等概念，使用加粗更合适，避免内联代码导致的代码块渲染问题。
